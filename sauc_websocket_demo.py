...
class Config:
    def __init__(self):
        # 生产环境建议改为读取环境变量或安全配置中心，不要硬编码
        self.auth = {
            "app_key": "5142285262",                     # APP ID
            "access_key": "3hpVlzSZZkLakcOEMsfKDcDDWWdKCxpb",  # Access Token
            "secret_key": "XvE8oA0RSecmzf5RB45Ln3eTvNQFDT8b"    # Secret Key（当前协议未用到，保留）
        }
        # 资源 ID：小时版/并发版（二选一）。默认小时版，若你开通并发版请改为 'volc.bigasr.sauc.concurrent'
...
class RequestBuilder:
    ...
    def new_full_client_request(seq: int) -> bytes:  # 添加seq参数
        ...
        # 最高准确率：使用非双向流式模式（bigmodel_nostream），并开启 ITN/标点/DDC
        payload = {
            "user": {
                "uid": "demo_uid"
            },
            "audio": {
                "format": "pcm",   # 发送纯音频数据时，使用 pcm/raw（不要填 wav）
                "codec": "raw",
                "rate": 16000,
                "bits": 16,
                "channel": 1
            ...
            "request": {
                ...
                "enable_punc": True,
                "enable_ddc": True,
                "show_utterances": False,    # 非双向流式场景下，通常只关心最终结果
                "enable_nonstream": True     # 配合 bigmodel_nostream 使用，提升准确率
            }
        }
    ...
    def new_audio_only_request(seq: int, segment: bytes, is_last: bool = False) -> bytes:
        header = AsrRequestHeader.default_header()
        header.with_serialization_type(SerializationType.NO_SERIALIZATION)  # 音频分包无JSON序列化
...
class AsrWsClient:
    ...
    async def create_connection(self) -> None:
        ...
        try:
            ...
            logid = None
            try:
                logid = self.conn.headers.get('X-Tt-Logid') or self.conn.headers.get('X-Tt-LogId')
            except Exception:
                pass
            connect_id = headers.get('X-Api-Connect-Id')
            if logid:
                logger.info(f"Connected to {self.url} (X-Tt-Logid={logid}, X-Api-Connect-Id={connect_id})")
            else:
                logger.info(f"Connected to {self.url} (X-Api-Connect-Id={connect_id})")
        except Exception as e:
    ...
    async def execute(self, file_path: str) -> AsyncGenerator[AsrResponse, None]:
        ...
        try:
            ...
            # 2. 提取WAV数据并计算分段大小（避免把WAV头一起切片发送）
            num_channels, samp_width, frame_rate, _, wave_data = CommonUtils.read_wav_info(content)
            size_per_sec = num_channels * samp_width * frame_rate
            segment_size = size_per_sec * self.segment_duration // 1000
            ...
            # 5. 启动音频流处理（仅发送纯音频数据部分）
            async for response in self.start_audio_stream(segment_size, wave_data):
                yield response
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="ASR WebSocket Client (highest accuracy: nostream)")
    ...
    parser.add_argument("--url", type=str, default="wss://openspeech.bytedance.com/api/v3/sauc/bigmodel_nostream", 
                       help="WebSocket URL (default: bigmodel_nostream for highest accuracy)")
    ...
    # 同步传入的资源 ID 到运行配置
    config.resource_id = args.resource_id
